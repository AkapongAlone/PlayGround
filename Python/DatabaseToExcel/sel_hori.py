...
"")
    ...
    def gencheck(self):      #ฟังชั่นไว้เจ็นค่าจากdatabase มาเป็นcheck box
        ...
        print(self.Featname1, self.StationName,  self.PartName, str(self.datetime_start), str(self.datetime_end))
    ...
    def run(self):
        ...
        endcol=self.end_colunm
        startcol2 = self.start_colunm2
        endcol2 = self.end_colunm2
        startcol3=self.start_colunm3
        endcol3=self.end_colunm3
        startcol4=self.start_colunm4
        endcol4=self.end_colunm4
        startcol5 = self.start_colunm5
        endcol5 = self.end_colunm5
        interval=self.interval
        # row2=self.start_row2
        ndata =endcol-startcol+1
        ndata2 = endcol2-startcol2+1
        ndata3 =endcol3-startcol3+1
        ndata4 =endcol4-startcol4+1
        ndata5 = endcol5-startcol5+1
        list_data =[]
        ...
        for v, x in enumerate(list_check):
            if len(x) != 0:
                ...
                if len(x) != y:
                    ...
                    msg.setText("จำนวนข้อมูลของแถว "+" "+str(list_name[v])+" "+"ที่เลือกไม่ตรงกับข้อมูลที่ต้องบันทึก ข้อมูลที่ต้องการคือ "+str(y)+"จำนวนที่เลือกคือ "+str(len(x)))
            ...
            for y in list_check2:
                if y == select:
                    for t in range(0, len(select)):
                        try:
                            ws.cells(self.start_row, startcol +t*interval).value = str(select[t])
                        except:
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Information)
                    ...
                    for t in range(0, len(select2)):
                        try:
                            ws.cells(self.start_row2,
                            self.start_colunm2 + t*interval).value = str(select2[t])
                        except:
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Information)
                            msg.setText(
                    ...
                    for t in range(0, len(select3)):
                        try:
                            ws.cells(self.start_row3,self.start_colunm3 + t*interval).value = str(select3[t])
                        except:
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Information)
                            msg.setText(
                    ...
                    for t in range(0, len(select4)):
                        try:
                            ws.cells(self.start_row4,self.start_colunm4 + t*interval).value = str(select4[t])
                        except:
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Information)
                            msg.setText(
                    ...
                    for t in range(0, len(select5)):
                        try:
                            ws.cells(self.start_row5,self.start_colunm5 + t*interval).value = str(select5[t])
                        except:
                            msg = QMessageBox()
                            msg.setIcon(QMessageBox.Information)
                            msg.setText(
